...
describe('filterQuestions', () => {
  ...
  test('(ALL) filters questions by tag', () => {
    expect(
      titlesOfQuestions(
        filterQuestions(testData, 'any', ['3'], '', FilterType.ALL)
      )
  ...
  test('(ALL) filters questions by title', () => {
    expect(
      titlesOfQuestions(
        filterQuestions(testData, 'any', [], 'better', FilterType.ALL)
      )
    ).toEqual(['Why does everything sound better in triplets?'])
  })
  ...
  test('(ALL) filters questions by title and tag', () => {
    expect(
      titlesOfQuestions(
        filterQuestions(testData, 'any', ['3'], 'triplets', FilterType.ALL)
      )
    ).toEqual(['Why does everything sound better in triplets?'])
  })
  ...
  test('(ANY) filters questions such that it shows all results when no filters', () => {
    expect(
      titlesOfQuestions(
        filterQuestions(testData, 'any', [], '', FilterType.ANY)
      )).toEqual([
        'Why do I test things?',
        'What do I need to do before submitting my code?',
        'Why does everything sound better in triplets?'
      ])
  })
})
